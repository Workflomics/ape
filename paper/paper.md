...
<!-- Vedran: I added a section on APE v2.6 and REST APE after this one. We should probably shorten this section and/or integrate it with the "APE v2.6 and REST APE" section. -->
...
Therefore, we have developed the Automated Pipeline Explorer (APE) [@kasalica_2026_ape;@kasalica2020]
...
# APE v2.6 and REST APE

APE was originally introduced in [@kasalica2020] as a Java library with a programmatic API and a command-line interface (CLI). This paper presents two significant additions to the APE ecosystem: APE v2.6 [@kasalica_2026_ape] and REST APE [@kasalica_2025_restape].

APE v2.6 extends the original tool annotation support with compatibility for CWL tool descriptions [@amstutz_common_2016], enabling seamless integration with the large collection of CWL-annotated tools available in the bioinformatics domain. Furthermore, APE v2.6 can generate executable CWL workflows as output, allowing users to go from existing CWL tool descriptions directly to executable workflows. APE v2.6 also adds support for Snakemake as an additional output format, further broadening its applicability.

REST APE [@kasalica_2025_restape] complements the core library by exposing APE's functionality through a RESTful API, enabling workflow exploration via HTTP requests and facilitating integration into third-party systems. REST APE additionally provides a design-time benchmarking component that leverages external metadata sources, such as bio.tools [@ison_tools_2016] and OpenEBench [@Capella-Gutierrez181677], to assess the compatibility of candidate workflows at the tool level. Figure 1 shows the structure of a candidate proteomics pipeline generated by APE and rendered in the Workflomics web platform [@kasalica_2024_14016325]. Figure 2 shows the corresponding design-time benchmarking results for that workflow, as visualised by Workflomics using data provided by REST APE. REST APE also handles storage and management of domain models and problem specifications, simplifying project management for users.

![Workflow structure of a candidate proteomics pipeline generated by APE and rendered in the Workflomics web platform, illustrating the data flow from raw inputs to enrichment analysis output.\label{fig:workflow_structure}](Figures/workflomics_workflow_structure.png){ width=60% }

![Design-time benchmarking results for the same candidate workflow as visualised by the Workflomics web platform, covering OS compatibility, license openness, and cumulative citation count across the tools in the workflow.\label{fig:benchmarks}](Figures/workflomics_benchmarks.png){ width=50% }
