...
![Workflow structure of a candidate proteomics pipeline generated by APE and rendered in the Workflomics web platform, illustrating the data flow from raw inputs to enrichment analysis output.\label{fig:workflow_structure}](Figures/workflow_fig.png){ width=60% }

![Design-time benchmarking results for the same candidate workflow as visualised by the Workflomics web platform, covering OS compatibility, license openness, and cumulative citation count across the tools in the workflow.\label{fig:benchmarks}](Figures/workflow_benchmarks.png){ width=50% }
